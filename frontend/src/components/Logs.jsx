...
function Logs() {
	...
	useEffect(() => {
		...
		if (timeToBed && timeWakeUp) {
			...
			setSleepDurationDisplay(`${sleepHours.toString().padStart(2, "0")}:${sleepMinutes.toString().padStart(2, "0")}`)
	...
	return (
		...
		{[...Array(10)].map((_, index) => (
			<Button key={index} className={mood == index + 1 ? "active" : ""} style={{ backgroundColor: getColor(index) }} onClick={() => handleMoodChange(index + 1)}>
			{index + 1}
			</Button>
			))}
